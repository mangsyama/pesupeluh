...
import { Search, Eye, Calendar, MapPin, ChevronLeft, ChevronRight, Inbox, Clock, CheckCircle, Wrench, Play, AlertCircle } from '@lucide/vue';
...
// Set default tab to ASSIGNED (new tasks assigned to the technician)
const currentTab = ref(props.filters.status || 'ASSIGNED');
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60 text-sm text-slate-850 dark:text-slate-300">
                        ...
                        <tr v-for="ticket in tickets.data" :key="'ticket-' + ticket.id" class="hover:bg-slate-50/30 dark:hover:bg-slate-800/10 transition duration-150">
                            ...
                            <!-- Reporter / Room -->
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="font-medium text-slate-850 dark:text-slate-200 text-xs">{{ ticket.reporter?.name ?? '-' }}</div>
                            ...
                            <!-- Category -->
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="font-semibold text-slate-850 dark:text-slate-200 text-xs">
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between items-start gap-2">
                            <div class="space-y-0.5">
                                ...
                                <h4 class="font-bold text-slate-950 dark:text-white text-xs line-clamp-1">
                                    {{ ticket.category?.name ?? '-' }}
                                </h4>
                            </div>
