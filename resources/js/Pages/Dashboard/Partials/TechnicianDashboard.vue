...
const stats = computed(() => {
    ...
    if (!ds) {
        return [
            getStatConfig(null, 'total', 'Total Tugas Saya'),
            getStatConfig(null, 'progress', 'Dalam Pengerjaan'),
            getStatConfig(null, 'completed', 'Selesai Dikerjakan'),
            getStatConfig(null, 'pending', 'Ditangguhkan'),
        ];
    }

    return [
        getStatConfig(ds.stat1, 'total', 'Total Tugas Saya'),
        getStatConfig(ds.stat2, 'progress', 'Dalam Pengerjaan'),
        getStatConfig(ds.stat3, 'completed', 'Selesai Dikerjakan'),
        getStatConfig(ds.stat4, 'pending', 'Ditangguhkan'),
    ].filter(Boolean);
});
...
<template>
    <div class="space-y-4">
        ...
        <!-- Akses Cepat Layanan -->
        <div class="bg-white dark:bg-slate-900 border border-white dark:border-slate-800 rounded-2xl p-5 sm:p-6 shadow-sm space-y-4">
        ...
        <!-- Content Rows: Aktivitas Tugas Terbaru + Distribusi Kategori Tugas -->
                    ...
                    <div class="flex items-start justify-between gap-3 mb-4">
                        <div>
                            ...
                            <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                                Daftar tiket penugasan yang memerlukan tindakan penanganan teknisi
                            </p>
                        </div>
                        <Link 
                        ...
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60 text-xs text-slate-700 dark:text-slate-300">
                                ...
                                <template v-if="isLoading">
                                    <tr v-for="n in 4" :key="'skel-tech-' + n" class="align-middle">
                                ...
                                <tr v-else-if="recentReports.length === 0">
                                    <td colspan="5" class="py-8 text-center text-slate-400 dark:text-slate-500">
                                        {{ __('Belum ada tugas tiket yang ditugaskan.') }}
                                    </td>
                                </tr>
                                ...
                                <tr v-else v-for="report in recentReports" :key="report.id" class="align-middle hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                    <td class="py-3.5 pr-4 whitespace-nowrap">
                                        <Link 
                                            ...
                                            :title="`Buka lembar kerja tiket #${report.id}`"
                                        >
                                            #{{ report.id }}
                                        </Link>
            ...
            <div class="lg:col-span-1 bg-white dark:bg-slate-900 border border-transparent dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col justify-between space-y-4">
                <div>
                    ...
                    <div class="space-y-4">
                        ...
                        <template v-if="isLoading">
                            <div v-for="n in 4" :key="'skel-cat-tech-' + n" class="space-y-1">
                                <div class="flex items-center justify-between">
                        ...
                        <div v-else-if="categoriesBreakdown.length === 0" class="text-center py-8 text-xs text-slate-400 dark:text-slate-500 font-semibold">
                            {{ __('Belum ada distribusi data tugas.') }}
                        </div>
                        ...
                        <div v-else v-for="category in categoriesBreakdown" :key="category.name" class="space-y-1">
                            <div class="flex items-center justify-between text-xs font-semibold text-slate-700 dark:text-slate-300">
                                ...
                                <span class="font-semibold">{{ category.count }} Tugas</span>
                            </div>
                            <div class="w-full bg-slate-100 dark:bg-slate-800 h-2 rounded-full overflow-hidden">
                                <div 
                ...
                <div class="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs text-slate-400">
                    <span>Kategori Tugas Ditangani</span>
                    <span class="font-bold text-slate-700 dark:text-slate-300">5 Terbesar</span>
                </div>
            </div>
