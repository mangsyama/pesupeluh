...
import { getDisplayDescription } from '@/Utils/sipuasHelper';
...
const stats = computed(() => {
    ...
    if (!ds) {
        return [
            getStatConfig(null, 'total', 'Total Laporan Saya'),
    ...
    return [
        getStatConfig(ds.stat1, 'total', 'Total Laporan Saya'),
...
<template>
    <div class="space-y-4">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                        Pilih unit layanan penunjang untuk akses cepat ke formulir pelaporan yang dibutuhkan
                    </p>
                </div>
            </div>
        ...
        <!-- Content Rows: Aktivitas Laporan Saya + Volume Kategori -->
                    ...
                    <div class="flex items-start justify-between gap-3 mb-4">
                        <div>
                            <h4 class="text-base font-bold text-slate-900 dark:text-white leading-tight">
                                Aktivitas Laporan Saya Terbaru
                            </h4>
                            <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                                Pantau status penanganan laporan kendala sarana prasarana yang Anda ajukan
                            </p>
                        </div>
                        <Link 
                        ...
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60 text-xs text-slate-700 dark:text-slate-300">
                                ...
                                <template v-if="isLoading">
                                    <tr v-for="n in 4" :key="'skel-staff-' + n" class="align-middle">
            ...
            <div class="lg:col-span-1 bg-white dark:bg-slate-900 border border-transparent dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col justify-between space-y-4">
                <div>
                    ...
                    <div class="space-y-4">
                        ...
                        <template v-if="isLoading">
                            <div v-for="n in 4" :key="'skel-cat-staff-' + n" class="space-y-1">
                                <div class="flex items-center justify-between">
                        ...
                        <div v-else v-for="category in categoriesBreakdown" :key="category.name" class="space-y-1">
                            <div class="flex items-center justify-between text-xs font-semibold text-slate-700 dark:text-slate-300">
                                ...
                                <span class="font-semibold">{{ category.count }} {{ __('pages.dashboard.reports_count') }}</span>
                            </div>
                            <div class="w-full bg-slate-100 dark:bg-slate-800 h-2 rounded-full overflow-hidden">
                                <div 
