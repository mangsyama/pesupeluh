...
const resumeTicket = () => {
    if (!props.ticket?.uuid || isResuming.value) return;
...
<template>
                                ...
                                <div v-else-if="ticket.status === 'PENDING'" class="space-y-3">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        <button
                                            ...
                                            :disabled="isResuming"
                                            :class="[
                                                'h-11 text-xs font-bold rounded-xl text-white dark:text-slate-900 flex items-center justify-center gap-1.5 bg-emerald-600 hover:bg-emerald-500 dark:bg-white dark:hover:bg-slate-200 transition duration-200',
                                                isResuming ? 'opacity-60 cursor-not-allowed' : ''
                                            ]"
                                        >
                                            <Play v-if="!isResuming" class="h-4.5 w-4.5" />
                                            <span>{{ isResuming ? __('Memproses...') : __('pages.tickets.detail.btn_resume') }}</span>
                                        </button>
        ...
        <div class="space-y-2">
            ...
            <div v-else-if="ticket.status === 'PENDING'" class="space-y-2">
                <button
                    ...
                    :disabled="isResuming"
                    :class="[
                        'w-full h-12 text-xs font-extrabold rounded-2xl text-white dark:text-slate-900 flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 active:scale-[0.98] dark:bg-white dark:hover:bg-slate-200 transition duration-200',
                        isResuming ? 'opacity-60 cursor-not-allowed' : ''
                    ]"
                >
                    <Play v-if="!isResuming" class="h-4.5 w-4.5" />
                    <span>{{ isResuming ? 'Memproses...' : 'Lanjutkan Pekerjaan' }}</span>
                </button>
                <button
                    @click="openCancelModal"
    ...
    <Modal :show="showArriveModal" @close="showArriveModal = false" max-width="lg">
        <div class="w-full flex flex-col bg-white dark:bg-slate-900">
            ...
            <div class="bg-emerald-600 dark:bg-emerald-950/90 text-white p-4 sm:p-5 flex items-center justify-between sticky top-0 z-10 shrink-0 border-b border-emerald-500/30 dark:border-emerald-800/50 shadow-sm">
                ...
                <button type="button" @click="showArriveModal = false" class="text-white/80 hover:text-white p-1.5 rounded-xl hover:bg-white/10 transition shrink-0">
                    <X class="h-5 w-5" />
                </button>
            </div>

            <form @submit.prevent="submitArrive" class="p-5 sm:p-6 space-y-4">
                ...
                <div class="flex justify-end gap-2 pt-2">
                    <SecondaryButton type="button" @click="showArriveModal = false" :disabled="arriveForm.processing || isSubmittingArrive">{{ __('Batal') }}</SecondaryButton>
                    <PrimaryButton type="submit" :disabled="arriveForm.processing || isSubmittingArrive || arrivePreviews.length === 0" :class="['!bg-emerald-600 hover:!bg-emerald-500', (arriveForm.processing || isSubmittingArrive) ? 'opacity-60 cursor-not-allowed' : '']">
                        {{ arriveForm.processing || isSubmittingArrive ? __('Menyimpan...') : __('Konfirmasi Kedatangan') }}
                    </PrimaryButton>
                </div>
            </form>
        </div>
    ...
    <Modal :show="showCompleteModal" @close="showCompleteModal = false" max-width="lg">
        <div class="w-full flex flex-col bg-white dark:bg-slate-900">
            ...
            <div class="bg-emerald-600 dark:bg-emerald-950/90 text-white p-4 sm:p-5 flex items-center justify-between sticky top-0 z-10 shrink-0 border-b border-emerald-500/30 dark:border-emerald-800/50 shadow-sm">
                ...
                <button type="button" @click="showCompleteModal = false" class="text-white/80 hover:text-white p-1.5 rounded-xl hover:bg-white/10 transition shrink-0">
                    <X class="h-5 w-5" />
                </button>
            </div>

            <form @submit.prevent="submitResolve('COMPLETED')" class="p-5 sm:p-6 space-y-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea
                        ...
                        class="w-full p-3.5 border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-200 text-xs focus:ring-2 focus:ring-emerald-500 focus:outline-none min-h-[100px]"
                    ></textarea>
                ...
                <div class="space-y-2.5">
                    ...
                    <div v-else class="space-y-2">
                        ...
                        >
                            ...
                            <p class="text-xs font-bold text-slate-700 dark:text-slate-200">Unggah 1 Foto Bukti Hasil Penanganan</p>
                            <p class="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">JPG, PNG (Maks 5MB)</p>
                        </div>
                ...
                <div class="flex justify-end gap-2 pt-2">
                    <SecondaryButton type="button" @click="showCompleteModal = false" :disabled="resolveForm.processing || isSubmittingResolve">{{ __('Batal') }}</SecondaryButton>
                    <PrimaryButton type="submit" :disabled="resolveForm.processing || isSubmittingResolve" :class="['!bg-emerald-600 hover:!bg-emerald-500 font-bold', (resolveForm.processing || isSubmittingResolve) ? 'opacity-60 cursor-not-allowed' : '']">
    ...
    <Modal :show="showPendingModal" @close="showPendingModal = false" max-width="lg">
        <div class="w-full flex flex-col bg-white dark:bg-slate-900">
            ...
            <div class="bg-amber-500 dark:bg-amber-950/90 text-white p-4 sm:p-5 flex items-center justify-between sticky top-0 z-10 shrink-0 border-b border-amber-400/30 dark:border-amber-800/50 shadow-sm">
                ...
                <button type="button" @click="showPendingModal = false" class="text-white/80 hover:text-white p-1.5 rounded-xl hover:bg-white/10 transition shrink-0">
                    <X class="h-5 w-5" />
                </button>
            </div>

            <form @submit.prevent="submitResolve('PENDING')" class="p-5 sm:p-6 space-y-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea
                        ...
                        class="w-full p-3.5 border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-200 text-xs focus:ring-2 focus:ring-amber-500 focus:outline-none min-h-[100px]"
                    ></textarea>
                    <div v-if="resolveForm.errors.notes" class="text-[10px] text-red-500 font-semibold">{{ resolveForm.errors.notes }}</div>
                </div>

                <div class="flex justify-end gap-2 pt-2">
                    <SecondaryButton type="button" @click="showPendingModal = false" :disabled="resolveForm.processing || isSubmittingResolve">{{ __('Batal') }}</SecondaryButton>
                    <PrimaryButton type="submit" :disabled="resolveForm.processing || isSubmittingResolve" :class="['!bg-amber-500 hover:!bg-amber-450 font-bold', (resolveForm.processing || isSubmittingResolve) ? 'opacity-60 cursor-not-allowed' : '']">
    ...
    <Modal :show="showCancelModal" @close="showCancelModal = false" max-width="lg">
        <div class="w-full flex flex-col bg-white dark:bg-slate-900">
            ...
            <div class="bg-rose-600 dark:bg-rose-950/90 text-white p-4 sm:p-5 flex items-center justify-between sticky top-0 z-10 shrink-0 border-b border-rose-500/30 dark:border-rose-800/50 shadow-sm">
                ...
                <button type="button" @click="showCancelModal = false" class="text-white/80 hover:text-white p-1.5 rounded-xl hover:bg-white/10 transition shrink-0">
                    <X class="h-5 w-5" />
                </button>
            </div>

            <form @submit.prevent="submitResolve('CANCEL')" class="p-5 sm:p-6 space-y-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea
                        ...
                        class="w-full p-3.5 border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-200 text-xs focus:ring-2 focus:ring-rose-500 focus:outline-none min-h-[100px]"
                    ></textarea>
                    <div v-if="resolveForm.errors.notes" class="text-[10px] text-red-500 font-semibold">{{ resolveForm.errors.notes }}</div>
                </div>

                <div class="flex justify-end gap-2 pt-2">
                    <SecondaryButton type="button" @click="showCancelModal = false" :disabled="resolveForm.processing || isSubmittingResolve">{{ __('Batal') }}</SecondaryButton>
                    <DangerButton type="submit" :disabled="resolveForm.processing || isSubmittingResolve" :class="['font-bold', (resolveForm.processing || isSubmittingResolve) ? 'opacity-60 cursor-not-allowed' : '']">
