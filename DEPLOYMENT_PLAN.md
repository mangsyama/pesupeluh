...
## Overview
This document outlines the exact steps to build and launch the three‑container stack (**PHP 8.3‑FPM**, **Nginx**, **Redis**) using the `docker-compose.yml` already present in the project root. All containers are given explicit, easy‑to‑read names:
- `pesupeluh-php`
- `pesupeluh-nginx`
- `pesupeluh-redis`
...
|-------|---------|-----------------|
| Containers up | `docker compose ps` | All three show `State: Up` |
| PHP version | `docker exec pesupeluh-php php -v` | `PHP 8.3.x` |
| Nginx serving | Open `http://localhost` in a browser | Your Laravel app (or default Nginx page) |
